...
if args.balanced:
   clf = VotingEnsembleLGBM(n_voters=args.voters, n_estimators=args.estimators, learning_rate=args.lr,
                            n_jobs=args.jobs, 
...
def classify(nfolds, repeat, jobs, verbose):
  if jobs == 1:
    ...
    result = [k_fold_cv(df_X, df_y, clf, n_splits=nfolds, seed=rseed, show_progress=True, verbose=verbose) for rseed in range(repeat)]
  else:
    if jobs == -1:
        print(f'Running par on {os.cpu_count()} cpus...')
    else:
        print(f'Running par on {jobs} cpus...')
    result = Parallel(n_jobs=jobs)(delayed(k_fold_cv)(df_X, df_y, clf, n_splits=nfolds, seed=rseed, show_progress=True, verbose=verbose) for rseed in range(repeat))
  return result
...
#ofile.write(f'PROBL: {" vs ".join([str(x) for x in np.unique(df_y.values)])}\n')
