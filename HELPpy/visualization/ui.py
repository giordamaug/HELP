    ...
    def fc6_change_title(fc4):
        if fc6.selected != ():
            try:
                with out72:
                    out72.clear_output()
                    val.value['df_x'] = load_features(list(fc6.selected), fixnas=[True]*len(list(fc6.selected)), normalizes=['std']*len(list(fc6.selected)), verbose=verbose, show_progress=show_progress)
