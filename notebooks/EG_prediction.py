...
parser.add_argument('-pl', "--poslabel", dest='poslabel', metavar='<poslabel>', type=int, help='positive label (default: 1)' , default=1, required=False)
parser.add_argument('-fx', "--fixna", action='store_true', default=False, help='enable fixing NaN (default disabled)', required=False)
parser.add_argument('-c', "--constrm", action='store_true', default=False, help='enable constant variable removal (default disabled)', required=False)
...
parser.add_argument('-e', "--estimators", dest='estimators', metavar='<voters>', type=int, help='n. of estimators (default: 100)' , default=100, required=False)
parser.add_argument('-lr', "--lr", dest='lr', metavar='<learningrate>', type=float, help='learning rate (default: 0.1)' , default=0.1, required=False)
...
myscorer = { 'ROC-AUC':      (roc_auc_score, {'multi_class':'ovr', 'average':'macro'}),
             ...
             'G-mean'  :     (geometric_mean_score , {'pos_label': args.poslabel}),
             'Specificity' : (specificity_score, {'pos_label': args.poslabel}),
             'Sensitivity' : (recall_score   , {'pos_label': args.poslabel}),
             'MCC' :         (matthews_corrcoef, {}), 
            }

clf = sveLGBM(n_voters=args.voters, n_estimators=args.estimators, learning_rate=args.lr,
                            n_jobs=args.jobs, 
...
for attrfile in args.inputfile:
   fixna = args.fixna
   constrm = args.constrm
   normalization = False if args.normalize is None else args.normalize
   if 'Emb' in os.path.basename(attrfile):
      fixnans += [False]  # no fixnan for embedding
      normalizes += [False]  # no normalization for embedding
      constrms += [False]  # no reove constants for embedding
   else:
      fixnans += [fixna]  # no fixnan for embedding
      normalizes += [normalization]  # no normalization for embedding
      constrms += [constrm]  # no reove constants for embedding

df_X = load_features(filenames=args.inputfile, fixnans=fixnans, normalizes=normalizes, constrms=constrms, verbose=verbose)
idxs = np.intersect1d(df_lab.index.values, df_X.index.values)
df_X = df_X.loc[idxs]
df_lab = df_lab.loc[idxs]

def classify(nfolds, repeat, jobs, verbose):
  if jobs == 1:
    print(f'Running seq on 1 cpu...')
    result = [skfold_cv(df_X, df_lab, clf, n_splits=nfolds, seed=rseed, show_progress=True, verbose=verbose, scorer=myscorer) for rseed in range(repeat)]
  else:
    if jobs == -1:
        print(f'Running par on {os.cpu_count()} cpus...')
    else:
        print(f'Running par on {jobs} cpus...')
    result = Parallel(n_jobs=jobs)(delayed(skfold_cv)(df_X, df_lab, clf, n_splits=nfolds, seed=rseed, show_progress=True, verbose=verbose, scorer=myscorer) for rseed in range(repeat))
  return result
...
import sys
distrib = np.unique(df_lab[label_name].values, return_counts=True)
ofile = sys.stdout if args.outfile is None else open(args.outfile, "a")
ofile.write(f'METHOD: {type(clf).__name__}\n')
ofile.write(f'PARAMS: {clf.get_params()} {clf.base_estimator.get_params()}\n')
